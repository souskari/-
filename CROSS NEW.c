...
char* getNextWord(TrieNode* root) {
    char word[50] = { 0 }; // статическая переменная для хранения текущего слова
    int index = 0; // индекс текущего слова
    ...
    if (index == 0) { // если это первый проход цикла
        word[0] = root->letter; // задаем первую букву текущего слова
        index++;
        return word;
    }

    // если не первый проход, ищем следующую букву в дереве
    TrieNode* current = root;
    int i = 0;
    ...
    if (i == index) { // если нашли следующую букву
        word[index] = current->letter;
        index++;
    }
    else { // если достигли конца слова
        word[index] = '\0';
        index = 0;
    }
...
int set_board(int words_count, struct TrieNode* root, char** board_copy, int bh, int bw, int f) {
    ...
    */
    ...
    int crossing = 0;
    int flag = f;
    char *tmp_word = malloc(sizeof(char)*30); //строка с текущим словом
    int k = 0;
    if (flag == 0) {
        
        ////////////// ПОЧТИ ПОРАВИЛА ///////////////////////////////////////////////////////////////////////
        for (; k < words_count || root_copy != NULL; tmp_word=getNextWord(root_copy)) {
            int tmp_i = 0;
            int tmp_j = 0;
            int orient = -1;
...
int min_cross_max_area(int words_count, TrieNode* root, char** board, int bh, int bw) {
    ...
    for (int k = 0; k < words_count; k++) {
        ...
        int flag_orient = 1;
        char *tmp_word = malloc(sizeof(char)*30);
        tmp_word = getNextWord(tmp_root);
        for (int j = 0; j < strlen(tmp_word); j++) {
            board_copy[(bh - strlen(tmp_word) )/ 2 + j][bw / 2] = tmp_word[j];
        }
        delete(tmp_root, tmp_word, 0, strlen(tmp_word));
