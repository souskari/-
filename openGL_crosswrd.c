...
// ќпределение структуры узла дерева
typedef struct TrieNode {
    struct TrieNode* children[ALPHABET_SIZE]; // массив указателей на потомков
    bool isEndOfWord; // флаг, указывающий, €вл€етс€ ли данный узел концом слова
    int size; //длина слова
} TrieNode;

// ‘ункци€ дл€ создани€ узла дерева
TrieNode* getNode() {
    TrieNode* node = (TrieNode*)malloc(sizeof(TrieNode)); // выделение пам€ти под узел
    node->isEndOfWord = false; // инициализаци€ флага
    for (int i = 0; i < ALPHABET_SIZE; i++) {
        node->children[i] = NULL; // инициализаци€ массива потомков
    }
    return node;
}

// ‘ункци€ дл€ вставки слова в дерево
void insertWord(TrieNode* root, const char* word) {
    int length = strlen(word); // вычисление длины слова
    TrieNode* currentNode = root; // начинаем с корневого узла
    for (int i = 0; i < length; i++) {
        int index = word[i] - 'A'; // вычисление индекса текущей буквы в алфавите
        if (currentNode->children[index] == NULL) { // если потомок с такой буквой не существует
            currentNode->children[index] = getNode(); // создаем новый узел
        }
        currentNode = currentNode->children[index]; // переходим к следующему узлу
    }
    currentNode->isEndOfWord = true; // помечаем текущий узел как конец слова
}

// ‘ункци€ дл€ поиска слова в дереве
bool searchWord(TrieNode* root, const char* word) {
    ...
    return (currentNode != NULL && currentNode->isEndOfWord); // возвращаем результат поиска
}

/* ¬џ„»—Ћ≈Ќ»≈ ƒЋ»Ќџ  ј∆ƒќ√ќ —Ћќ¬ј */ //вычисл€ет длину каждого слова и записывает в соответствующую €чейку (в €чейку последней буквы в слове)
void calculateSize(TrieNode* currentNode) {
    if (currentNode == NULL) {
        return;
    }
    for (int i = 0; i < ALPHABET_SIZE; i++) {
        calculateSize(currentNode->children[i]);
    }
...
// ѕолучение следующего слова из префиксного дерева
bool getNextWord(TrieNode* root, char* word) {
    ...
    if (root->isEndOfWord && currentIndex > 0) {
        word[currentIndex] = '\0'; // ƒобавл€ем символ окончани€ слова
        return true;
    }
    ...
    for (int i = 0; i < ALPHABET_SIZE; i++) {
        if (source->children[i] != NULL) {
            if (destination->children[i] == NULL) {
                destination->children[i] = getNode();
            }
            copyTrie(source->children[i], destination->children[i]);
        }
    }
    destination->isEndOfWord = source->isEndOfWord;
}

//удаление слова
int delete(TrieNode** currentNode, char* word, int level, int length) {
    if (*currentNode != NULL) {
        if (level == length) {
            if ((*currentNode)->isEndOfWord) {
        ...
        if (delete(&(*currentNode)->children[index], word, level + 1, length) != 0) {
            free((*currentNode)->children[index]);
    ...
    if (j_board - pos + len + 1 < bw && tek_pos_j > 0 && i_board > 0 && (board_copy1[i_board][tek_pos_j + len] == ' ' && board_copy1[i_board][tek_pos_j - 1] == ' ')) {
        for (int i = 0; i < len && gorizont; i++) {
            if (board_copy1[i_board][tek_pos_j + i] == ' ' || board_copy1[i_board][tek_pos_j + i] == str[i]) { //если место куда хотим поставить очередную букву\
                ...
                if (board_copy1[i_board][tek_pos_j + i] == str[i]) { //если совпадает с буквой, то плюсуем пересечени€
                    gperes++;
                }
...
int set_board(int words_count, int* len_of_words1, struct TrieNode* root, char** board_copy, int bh, int bw, int f) {
    ...
    struct TrieNode* root_copy = getNode();
    /*

    функци€ котора€ копирует одно дерево в другое | из root в root_copy
    ...
    */
    ...
    if (flag == 0) {
        int k = 0;
        ////////////// ѕќ„“» ѕќ–ј¬»Ћј ///////////////////////////////////////////////////////////////////////
        for (; k < words_count; getNextWord(root, tmp_word)) {
            int tmp_i = 0;
            int tmp_j = 0;
            int orient = -1;
    ...
    ///////////////////////////////////////////////////////////////////////////////////
        ...
        for (; root_copy != NULL; getNextWord(root, tmp_word)) {
...
int main() {
    ...
    printf("¬рем€ выполнени€ программы: %f секунд\n", cpu_time_used);
    GLFWwindow* window;
    if (!glfwInit())
        return -1;
